import { c as create_ssr_component, o as onDestroy, h as spread, j as escape_object, f as createEventDispatcher } from './index3-UOcDFV3P.js';
...
function getIconData$1(data, name, full = false) {
  function getIcon(name2, iteration) {
    ...
    if (aliases && aliases[name2] !== undefined) {
    ...
    if (!iteration && chars && chars[name2] !== undefined) {
  ...
  if (result) {
    for (const key in iconDefaults) {
      if (result[key] === undefined && data[key] !== undefined) {
        result[key] = data[key];
      }
    }
  }
...
function quicklyValidateIconSet(obj) {
  ...
  for (const prop in optionalProperties) {
    if (obj[prop] !== undefined && typeof obj[prop] !== optionalProperties[prop]) {
      return null;
    }
  }
  const icons = data.icons;
  for (const name in icons) {
    ...
    for (const prop in iconDefaults) {
      if (icon[prop] !== undefined && typeof icon[prop] !== typeof iconDefaults[prop]) {
        return null;
      }
    }
  }
...
function getStorage(provider, prefix) {
  if (storage$1[provider] === undefined) {
    storage$1[provider] = /* @__PURE__ */ Object.create(null);
  }
  const providerStorage = storage$1[provider];
  if (providerStorage[prefix] === undefined) {
...
function createAPIConfig(source) {
  ...
  const result = {
    resources,
    path: source.path === undefined ? "/" : source.path,
...
function getPath(provider) {
  if (typeof provider === "string") {
    if (pathCache[provider] === undefined) {
      const config2 = getAPIConfig(provider);
      if (!config2) {
        return "/";
      }
...
function sortIcons(icons) {
  ...
  icons.forEach((icon) => {
    ...
    if (storage2[provider] === undefined) {
    ...
    let list;
    if (localStorage.icons[name] !== undefined) {
      list = result.loaded;
    } else if (prefix === "" || localStorage.missing[name] !== undefined) {
      list = result.missing;
    } else {
      list = result.pending;
    }
...
function updateCallbacks(provider, prefix) {
  if (pendingUpdates[provider] === undefined) {
  ...
  if (!providerPendingUpdates[prefix]) {
    ...
    setTimeout(() => {
      ...
      if (callbacks[provider] === undefined || callbacks[provider][prefix] === undefined) {
        return;
      }
      ...
      items.forEach((item) => {
        ...
        icons.pending = icons.pending.filter((icon) => {
          ...
          const name = icon.name;
          if (storage2.icons[name] !== undefined) {
            icons.loaded.push({
              provider,
              prefix,
              name
            });
          } else if (storage2.missing[name] !== undefined) {
            icons.missing.push({
              provider,
              prefix,
              name
...
function getRedundancyCache(provider) {
  if (redundancyCache[provider] === undefined) {
    const config2 = getAPIConfig(provider);
    if (!config2) {
      return;
    }
...
function loadedNewIcons(provider, prefix) {
  if (loaderFlags[provider] === undefined) {
...
function loadNewIcons(provider, prefix, icons) {
  ...
  if (iconsToLoad[provider] === undefined) {
    iconsToLoad[provider] = /* @__PURE__ */ Object.create(null);
  }
  const providerIconsToLoad = iconsToLoad[provider];
  if (queueFlags[provider] === undefined) {
    queueFlags[provider] = /* @__PURE__ */ Object.create(null);
  }
  const providerQueueFlags = queueFlags[provider];
  if (pendingIcons[provider] === undefined) {
    pendingIcons[provider] = /* @__PURE__ */ Object.create(null);
  }
  const providerPendingIcons = pendingIcons[provider];
  if (providerIconsToLoad[prefix] === undefined) {
...
const loadIcons = (icons, callback) => {
  ...
  sortedIcons.pending.forEach((icon) => {
    ...
    const providerPendingIcons = pendingIcons[provider];
    if (providerPendingIcons[prefix] === undefined) {
      providerPendingIcons[prefix] = /* @__PURE__ */ Object.create(null);
    }
    if (newIcons[provider] === undefined) {
      newIcons[provider] = /* @__PURE__ */ Object.create(null);
    }
    const providerNewIcons = newIcons[provider];
    if (providerNewIcons[prefix] === undefined) {
  ...
  sortedIcons.pending.forEach((icon) => {
    ...
    if (pendingQueue[name] === undefined) {
...
if (typeof document !== "undefined" && typeof window !== "undefined") {
  ...
  if (_window2.IconifyPreload !== undefined) {
  ...
  if (_window2.IconifyProviders !== undefined) {
...
export { Icon as I, enableCache as e, loadIcons as l };
//# sourceMappingURL=Icon-Dg-p2Y8u.js.map
